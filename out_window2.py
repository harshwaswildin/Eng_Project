...
class Ui_OutputDialog(QDialog):
    ...
    def runClockIn(self):
        ...
        if name!="unknown":
            print(name,"has switched the laser off")
            f  = open("Attendance.csv", 'a')
            date_string = datetime.datetime.now().strftime("%d/%m/%y")
            time_string = datetime.datetime.now().strftime("%H:%M:%S")
            f.writelines(f'\n{name},{date_string},{time_string},SWITCHED OFF')
            capture.release()
            self.capture.release()
            app = s.Popen('python3 laser_on.py')
            s.Popen.terminate(app)
            app = s.Popen('python3 onWindow.py')
            s.Popen.terminate(app)
            f.close()
            self.close()
            #subprocess.run(["python", "onWindow.py"])
            #subprocess.run(["python", "laser_on.py"])
            #os.system(f'python C:/Users/rkail/OneDrive/Desktop/LaserTripwireProject/mainwindow.py')
            a = threading.Thread(target=execute_script, args=('mainwindow.py',))
            a.start()
            b = threading.Thread(target=execute_script, args=('laser_off.py',))
            b.start()
            a.join()
            b.join()
            os._exit(0)
